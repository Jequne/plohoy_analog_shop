...
from products_data import AdminInfo, Product
...
class AdminRouter(APIRouter):
    ...
    # async def login(self, password: str = Form(...), request: Request = None, db: Session = Depends(get_db)):
    #     """Обработка логина"""
    ...
    #     #     response.set_cookie(key="admin", value="true", httponly=True, max_age=28800)
    #     #     return response
    #     else:
    #         raise HTTPException(status_code=401, detail="Invalid credentials")
    async def login(self, request: Request=None, password: str = Form(...), db: Session = Depends(get_db)):
...
class PageRoutes(APIRouter):
    def __init__(self):
        ...
        self.add_api_route("/plohoy.shop", self.products_all, methods=["GET"], response_class=HTMLResponse)
        self.add_api_route("/About", self.about, methods=["GET"], response_class=HTMLResponse)        
        self.add_api_route("/terms-of-service", self.terms_serv, methods=["GET"], response_class=HTMLResponse)  
        self.add_api_route("/product", self.product, methods=["GET"], response_class=HTMLResponse)
...
class CartLogic(APIRouter):
    def __init__(self):
        ...
        self.templates = Jinja2Templates(directory="../assets/PP")
        self.add_api_route("/admin/products", self.admin_products, methods=["GET"])
        self.add_api_route("/admin/products/add", self.add_product, methods=["POST"])
        self.add_api_route("/admin/edit-or-no", self.edit_or_no, methods=["GET"])
        self.add_api_route("/admin/products/delete/{product_id}", self.delete_product, methods=["DELETE"])
        self.add_api_route("/admin/products/edit/{product_id}", self.edit_product, methods=["POST"])

    # async def admin_products(self, request: Request, db: Session = Depends(get_db)):
    #     if request.cookies.get("admin") != "true":
    #         raise HTTPException(status_code=401, detail="You didn't log in as admin!")
    #     products = db.query(Product).all()
    #     return self.templates.TemplateResponse("admin_products.html", {"request": request, "products":products})
    async def admin_products(self, request: Request, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
        products = db.query(Product).all()
        return self.templates.TemplateResponse("admin_products.html", {"request": request, "products": products})

    async def add_product(self, name: str = Form(...), price: int = Form(...), quantity: int = Form(...),\
                           db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
        if db.query(Product).filter(Product.name == name).first():
            response = RedirectResponse(url="/admin/edit-or-no", status_code=303)
            return response
            # return JSONResponse(content={"message": "product with both name already in"})
    ...
    async def delete_product(self, product_id: int, request: Request, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
        # if request.cookies.get("admin") != "true":
        #     raise HTTPException(status_code=401, detail="You didn't log in as admin!")
    ...
    async def edit_product(self, product_id: int, name: str = Form(...), price: int = Form(...), quantity: int = Form(...), db: Session = Depends(get_db)):
